export default {
  //內層
  props: ["pageies", "getData"], //內層定義props的pagies，讓外層的pagination傳進來且要去v-bind(口訣:前內(即props定義的名稱)後外(外層元件的變數名稱))
  ...
  /*emit的寫法 經由emit透過pagination元件(內層)觸發getData外層事件 */
  // 插入分頁modal
  /*
    ...
    <ul class="pagination">
      ...
      <a class="page-link" href="#" @click.prevent="$emit('change-page',page)">{{page}}</a> //emit step2:定義內層$emit觸發方法:自訂$emit的事件名稱'change-page'，參數帶入page
      </li>
